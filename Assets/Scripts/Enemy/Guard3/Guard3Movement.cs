...
public class Guard3Movement : MonoBehaviour {
	...
	public Guard3BulletFiring shoot;
	public Guard3Movement guardMovement;
	...
	void Start()
	{
		...
		seek.rdb = rdb;
		ranNumber = Random.Range (5, 10);
		targetPos = new Vector2(playerPivotPos.x - ranNumber, 0f);
	...
	void Update()
	{
		...
		if (playerHealth.isDead) {
			...
			gameObject.GetComponent<Guard3Movement>().enabled = false;
			shoot.enabled = false;
		}
	}//Update
	...
	void FixedUpdate()
	{
		...
		bool hitLeft = Physics2D.Raycast(new Vector2 (transform.position.x - 1f, transform.position.y), Vector2.left, 7f, playerMask);
		bool hitRight = Physics2D.Raycast(new Vector2 (transform.position.x + 1f, transform.position.y), Vector2.right, 7f, playerMask);
