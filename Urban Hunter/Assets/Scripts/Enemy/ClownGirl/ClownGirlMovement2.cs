...
public class ClownGirlMovement2 : MonoBehaviour {
	...
	void initialize()
	{
		...
		if (insert) {
			...
			dict.Add(3, new Point(5, 6));
			dict.Add(4, new Point(-1, -1));
			for(int i = 1;i < pathPoints.Length;i++){
				if(Vector2.Distance(girlTransform.position, pathPoints[i].position) <= 10f)
					currentPos = i;
			}
			insert = false;
		}
	...
	void ChooseDirection()
	{
		...
		if(distanceToPlayer >= 0f && distanceToPlayer <= 10f)
	...
	void jump()
	{
		if (faceRight) {
			if(currentPos == 1)
			{
				...
				targetPos = pathPoints[2].position;
			}
			if(currentPos == 2)
			{
				doNext = action.RUN_RIGHT;
				targetPos = pathPoints[3].position;
			}
			if(currentPos == 3)
			{
				doNext = action.RUN_RIGHT;
				targetPos = pathPoints[4].position;
			}
		}
		else {
			if(currentPos == 4)
			...
			{
				doNext = action.RUN_LEFT;
				targetPos = pathPoints[1].position;
			}
		}
	}//jump
}
